...
## V3 Watson Operating Rules (absorbed from STREAM-A-watson-operating-rules.md, 8-May-26)

These four rules govern Watson's behaviour when applying the V3 framework. They are operating rules for *how Watson acts*, not framework concepts. Apply alongside the Operating Anchors above.

- **W1 — Element-specific rating progression (D1).** Default = walk the in-scope elements per the per-row authoring matrix at the current stage; update ratings on high-value elements; carry forward all prior ratings unchanged. **Override:** if APM judgement during this stage's work surfaces a reason to re-rate a low-value element, do it (document with brief reason in the memo). **Don't force time/attention onto low-value re-rating;** focus it on the key topics the matrix specifies.

- **W2 — Broadly closed but ebb-and-flow element framework (Q6).** Treat E1–E32 as the closed universe of analytical questions. Set will ebb and flow over time as Richard learns and refines. **Watson is constrained from tinkering** — element changes require explicit "high value-add" justification and Richard's sign-off. If during real work a gap or duplication surfaces, flag it explicitly and ask Richard to decide.

- **W3 — "If it bubbles up" cognitive economy (D1).** Default = focus APM attention on key topics per the matrix. Don't force time onto low-value re-rating. **Override:** if work surfaces a reason to update a low-value element rating, do it. The framework's whole point is throughput — moving cases through the funnel rapidly. Cognitive overhead is the enemy. Once Pillar III is defined, APM mostly works from the in-flight checklist (per the plane analogy); re-engaging Pillars I + II is expensive and rare.

- **W4 — Stage gates are NOT programmatic (Q2/Q8).** Stage Target Conditions (TCs) are **qualitative alignment language**, not rated outcomes. APM does NOT rate the TCs. Go/no-go decisions come from the **combination** of element/group/pillar/master ratings + integrated APM judgement. There are no per-element acceptance thresholds. **The gate is APM judgement. Period.** Do not over-engineer a programmatic "if rating ≥ B then proceed" gate.

---
<!-- [W] Created 08-Apr-26 — Wave 1 completion. Comprehensive operational manual for Watson in APM mode. V3 framework patch 8-May-26 (Stream C1). -->
...
1. **This file** (SKILL-V3.md) — full read. Role definition, V3 framework + Watson operating rules, checklists, non-negotiables.
1a. **`projects/SA - Ratings Dashboard/new-framework-08may/STREAM-A-COMPREHENSIVE-V2.html`** — V3 framework canonical reference. Funnel view (§ 1), 6-stage funnel (§ 2), 32 A&J elements (§ 3), 23 RRs (§ 4), 6 master ratings + hierarchical layer (§ 5), Pillars I/II/III (§ 6), Pillar III tabular spec + tree (§ 7), plane analogy (§ 8), **per-row authoring matrix (§ 9 — load-bearing)**, Stage VI Live OS (§ 10), dashboard split (§ 11), strategic context (§ 12). Read for any A&J work.
1b. **`memory/skills/assistant-portfolio-manager/analysis-judgement-SOP-V3.md`** — V3 AJ SOP. Authoring order = stage-row-walking; effort allocation per-cell, not 70/30.
...
7. **memory/skills/notion-posting-standard/SKILL.md** — load before ANY Notion posting. Pipe tables, emphasis rules, colour convention, bullet splitting, verification checklist. This standard governs Notion rendering; APM governs content.
...
### A&J Memo Writing — Per-Row Authoring Matrix Execution Protocol [V3 framework, 8-May-26]

This block REPLACES the prior "Six Pillars IC Framework Execution Protocol" (V2, pre-8-May-26). The substance — APM's analytical hierarchy and rating discipline — is preserved; the **execution mechanism** is replaced with the **per-row authoring matrix** from `STREAM-A-COMPREHENSIVE-V2.html` § 9.

**The matrix IS the SOP.** APM produces the A&J memo by walking the rows of the matrix (xlsx Block B + Pillar III block, rows 45–125) top-to-bottom AT EACH STAGE, writing/updating content per the cell instruction at the (row × stage column) intersection.

#### Files to load in addition to standard APM loading protocol

1. **`STREAM-A-COMPREHENSIVE-V2.html`** — Stream A canonical framework. Contains: Funnel view (§ 1), 6-stage funnel (§ 2), 32 A&J elements (§ 3), 23 RRs (§ 4), 6 master ratings + hierarchical rating layer (§ 5), Pillars I/II/III (§ 6), Pillar III tabular spec + tree (§ 7), plane analogy (§ 8), **per-row authoring matrix (§ 9 — load-bearing)**, Stage VI Live OS (§ 10), dashboard split (§ 11), strategic context (§ 12).
2. **`STREAM-A-watson-operating-rules.md`** — W1–W4 operating rules absorbed into this file's operating rules section. Reference for cross-check.
3. **`databases/memo-schema.md`** — A&J memo schema (V2 patch pending Stream C2). A/B/C/D/E/F document spine survives; sections C and D internally reorganised around Pillar I/II/III.
4. **`memory/skills/researcher/SKILL-V2.md`** — RESEARCHER 23-RR framework, stage-bound. APM consumes only the RRs in scope at the current stage (per Funnel view § 1).
5. **`databases/master/ic-ratings-current.json`** — Master ratings database. Read before any analysis to check existing master/pillar/group/element ratings for the stock.
6. **`databases/monitoring/monitoring-plan.json`** — Active monitoring items (Stage VI Live).

#### Rating taxonomy — the hierarchy APM rates at every level [D2, hierarchical rating layer]

```
MASTER RATINGS (6) ← loosely-derived qualitative summaries (Watson option (b))
  ↓ informed by, not derived from
PILLAR ratings (I, II, III) ← APM judgement
  ↓
GROUP ratings (Pillar II's 5 groups; Pillar III's 3 groups)
  ↓
ELEMENT ratings (E1–E32) ← atomic A&J questions
  ↓
SUB-QUESTION ratings (where applicable, within elements)
```

**ALL levels are explicitly rated by APM.** Higher-level ratings are *informed by* but not *programmatically derived from* lower-level ratings. This is the judgement layer — where APM expertise adds value above mechanical roll-up. A Master Rating can be A even if one element is C (if APM judges the element non-load-bearing); it can be C even if all elements are B+ (if APM identifies a synthesis-level concern). Per `STREAM-A-COMPREHENSIVE-V2.html` § 5.

#### Vocabulary lock (post-8-May-26)

**OLD vocabulary → NEW vocabulary** (full glossary in `STREAM-A-COMPREHENSIVE-V2.html` § 12):

| Old | New | Notes |
|---|---|---|
| Pillars (P1–P6) | Master ratings (MR1–MR6) | Same 6 subjects, renamed |
| RAs (Required Attributes) | E# (A&J Elements) | Atomic analytical questions |
| CQs (Core Questions) | Sub-questions within an Element | No longer first-class |
| TCs (Target Conditions) | Stage TCs (qualitative, not rated) | Funnel-stage purpose, NOT rated |
| Pillar 3 (old: KDICs/KHFs/KCs/KRs/KPOs) | Pillar III (Position playbook + Monitoring plan + Key drivers) | New 3-group structure |
| Foundations (Pillar II G3) | Core philosophy | F67 rename per D4 |
| IC#1/2/3, BB#1–BB#8 (attribute families) | Pillar I (3 elements) + Pillar II (5 groups, 14 elements) | Reorganised |

Pillar III is structurally NEW — it is the operational distillation (in-flight checklist, per the plane analogy, § 8). Built post-DD; becomes operating layer at Stage VI Live.

#### The 6 Master Ratings

| # | Master rating | What it summarises |
|---|---|---|
| MR1 | Technical momentum | Needed for profitable risk/rewards |
| MR2 | Sell-side momentum | Needed for Stage 2 stocks to "keep going" |
| MR3 | Thematic fit | When long the right thematics, picking is easy |
| MR4 | Pillar I — Case change elements | Enough exciting change to drive SP up? |
| MR5 | Pillar II — Case building blocks | The investment change case + bankability |
| MR6 | Pillar III — Investment case components | In-flight playbook + monitoring plan |

#### Pillar I — Case change elements (3 elements)

Whether the case has enough exciting change to drive the share price.

- **E1** Required "input change forces" (internal/external)?
- **E2** Required case "outputs"? *(financials must improve, not just narrative)*
- **E3** Sufficient TSR / valuation upside?

#### Pillar II — Case building blocks (5 groups, 14 elements)

The bankability foundations — what makes change forces extrapolable mid/long-term.

| Group | Elements |
|---|---|
| **G1 Case momentum** | E4 Technical · E5 Thematic · E6 SS earnings momentum |
| **G2 Case simplicity** | E7 Simplicity guardrails · E8 Acceptable setup fit |
| **G3 Core philosophy** *(was Foundations)* | E9 Operator · E10 Advantaged + SRCA · E11 VC dynamics · E12 Industry structure · E13 Long-term growth |
| **G4 Case riskiness** | E15 Invalidating · E16 Seek-to-avoid · E17 AUM-constraining · E18 Clear transmission · E19 Monitorable transmission |
| **G5 Case optionality** | E20 Nice-to-have *[minor]* |

#### Pillar III — Investment case components (3 groups + 2 standalone elements)

The operational distillation — the in-flight checklist. Built post-DD; becomes operating layer at Stage VI Live.

| Group/Element | What it is |
|---|---|
| **E21 Monitoring plan** | Framework for monitoring once at Stage VI Live (links to RR23) |
| **E22 Position management playbook** | Entry rules, sizing, trim/add triggers, exit triggers |
| **G7 Key drivers Upstream (E23–E27)** | Input drivers: HFs, primary, secondary, risks, optionality |
| **G8 Key drivers Downstream (E28–E32)** | Output drivers: financial metrics, FSOs, risks, peers/sector, technical |

For Pillar III Groups 7 + 8, content is produced **tabularly per the per-component spec** (xlsx rows 106–125, full table in `STREAM-A-COMPREHENSIVE-V2.html` § 7a). For each driver, APM produces a row with 7 columns: I (what), J (LTI), K (count), L (modal), M (guidance), N (sell-side), O (invalidation threshold). The tree-diagram visualisation (§ 7b) is a rendering format; the table is the source of truth.

#### How APM executes A&J using the matrix

**The matrix is in `STREAM-A-COMPREHENSIVE-V2.html` § 9.** It maps rows (topics) × columns (stages) with cell instructions ("First pass", "Second pass — solid", "[no change]", "Rated", or blank for n.a.).

**Cell-instruction vocabulary:**

| Cell value | Instruction |
|---|---|
| (blank) / — | n.a. at this stage |
| First / Second / Third / Fourth / Fifth pass | Numbered re-engagement; depth grows with each pass |
| First / Second / Third pass — light / solid / robust | Standardised depth labels for groups + pillars |
| [no change] | Carry forward; do not re-touch at this stage |
| Rated | A rating is produced (master ratings column) |
| No | Explicitly do nothing at this stage |

**At each stage:**

1. Open the matrix at the column for the current stage (I Gen/screen · II Filter out · III Triage · IV ESA · V DD · VI Live)
2. Walk rows top-to-bottom
3. For each row with a non-blank cell, produce the content per cell instruction
4. Skip blank cells (n.a. at this stage)
5. Carry forward all prior content unchanged unless an instruction says to re-touch

**Worked example — at Stage III (Triage):**
- Row 48 (Exec summary judgement): produce *third pass* of exec summary
- Row 49 (Master ratings): produce *first pass — light* of all 6 master ratings
- Row 52 (E1 Required input change forces): *first pass*
- Row 56 (Pillar II): produce *first pass — light* of Pillar II rating
- Row 58 (G1 Case momentum): *first pass — light* of group rating
- Row 60 (E5 Thematic fit): *second pass* (was first-passed at Stage II)
- Rows 63 (G2 simplicity), 64 (E7), 65 (E8): all *first pass*
- Row 67 (G3 Core philosophy): *first pass — light*
- Row 70 (E11 VC dynamics): *second pass* (was first-passed at Stage II)
- Row 75 (G4 Case riskiness): *first pass — light*
- All other rows: blank → n.a. at this stage; skip

**That is the SOP.** The matrix dictates the deliverable. No 70/30 effort allocation rule (retired); no per-section word targets (retired). The cell instruction encodes both depth and effort.

#### Authoring order — stage-row-walking [REPLACES "C.II → A → D → C.I → B → E → F"]

Within a single stage, APM walks the rows top-to-bottom (= memo reading order):

1. Row 48 (exec summary judgement) — re-written first
2. Row 49 (master ratings) — updated next
3. Pillars I → II → III (depth-first within each in-scope element)

This replaces the prior within-section authoring order. The new SOP is **stage-row-walking**, not within-section reordering.

#### Two parallel outputs from Triage onwards [the "broadly" caveat]

Rows 48 + 49 are the *top of the matrix*, sitting alongside the deeper element work below. They give a **light, full-case summary** at every stage from Triage onwards, even when most pillars are still under-informed. APM produces both at once:

- **Top (light, full-case):** Executive summary judgement (row 48) + Master ratings (row 49) — written from Triage onwards, getting firmer with conviction at each subsequent stage even though many pillars are under-informed
- **Below (deep, stage-specific):** In-scope elements at this stage — depth-first, robust within scope, n.a. for out-of-scope

**One document, one matrix.** Light at the top + deep for in-scope elements below. Watson must NOT collapse the full-case summary into a purely sequential model.

#### Stage gates are NOT programmatic [W4]

Stage Target Conditions (TCs) are **qualitative alignment language**, not rated outcomes. APM does NOT rate the TCs. Go/no-go decisions come from the **combination** of element/group/pillar/master ratings at the stage. There are no per-element acceptance thresholds. **The gate is APM judgement. Period.**

This prevents over-engineering programmatic "if all in-scope ratings ≥ B then proceed" automation. The framework relies on integrated APM judgement at each gate.

---

### Signposting Doctrine (LOAD-BEARING — read before any C.II memo authoring) [D] (21-Apr-26, V3-vocabulary-patched 8-May-26)

> **V3 vocabulary note (8-May-26):** This block predates the new framework but its operational rules survive. Vocabulary mapping below applies to all signposting work in V3:
> - **CQ (Core Question)** → **Sub-question within an Element**
> - **RA (Required Attribute)** → **A&J Element (E#, where # = E1–E32 per Stream A § 3)**
...
> - **IC#1 / IC#2 / IC#3** → **Pillar I (Case change elements)** — three elements: E1 input change forces, E2 case outputs, E3 TSR upside
> - **BB#1–BB#8** → **Pillar II (Case building blocks)** — five groups: G1 momentum, G2 simplicity, G3 Core philosophy, G4 riskiness, G5 optionality (14 elements E4–E20)
> - **Pillar III** in old vocabulary referred to "What's Changing" (Pillar III) — in V3, **Pillar III** is now Investment case components (Position playbook + Monitoring plan + Key drivers); the old "What's Changing" maps to V3 Pillar I + Pillar II G1
> - **Section mapping** for the 26 deliverables list further below: A/B/C/D 4-section structure used here predates V3. Final reconciliation to V3 spine A/B/C/D/E/F happens in Stream C2 (memo schema patch). The deliverable list itself preserves operational value.
>
> The signposting RULES (5 numbered rules, two visual patterns prefix vs embedded, demi-bold weight 600, anti-patterns, JSON schema) survive intact — only the vocabulary in labels changes.

**Higher intent (the north star).** Every analytical statement APM writes in section C of the memo (Pillars I + II — case attributes) must be instantly traceable to the **A&J Element / Group / Pillar** it answers. When Richard reads a bullet, he must know in **<1 second** *why* he is reading it here-and-now. Without signposting, APM's analytical work is **wasted** because the surface fails to telegraph the analytical hierarchy.
...
### 1. The 6-Stage Funnel [V3, 8-May-26 — replaces old IG → ESA → DD → LIVE]

Richard's pipeline runs through **6 sequential stages** (not 4 or 5). Each stage has a discrete *primary purpose* and a *target condition*. Cases that fail a stage's target condition are **parked**; cases that clear graduate. Stages are **sequential filters, not deepening passes** (the old "onion peeling" model retires — every stage no longer covers the whole case at greater depth; instead, each stage focuses on its own specific elements).

| # | Stage | Roman | Primary purpose | Target condition (qualitative — NOT rated) |
|---|---|---|---|---|
| 1 | Generation / screening | I | Qualify a case | Sufficient technical momentum |
| 2 | Filtering out *(NEW)* | II | Filter out cases with obvious headwinds | Avoid avoidable losses by side-stepping large/worsening exogenous headwinds |
| 3 | Triaging *(first A&J pass)* | III | Lightly triage attractiveness | Require large positive change forces |
| 4 | ESA *(second A&J pass)* | IV | Broadly test attractiveness | Two-engine doctrine: change must show in financials AND case must be bankable |
| 5 | Deep dive *(third A&J pass)* | V | Robustly probe riskiness | Top-decile/quartile A/B only; mediocrity-shunning |
| 6 | Live | VI | Mitigate risk of capital loss | Use case components + LTI monitoring; assess monthly if tracking-to-plan or invalidated |

Full target conditions verbatim from xlsx (with mediocrity-shunning + two-engine doctrine detail) are in `STREAM-A-COMPREHENSIVE-V2.html` § 2.

**APM responsibilities at each stage:**
- **I Gen/screen:** No A&J memo. Confirm technical momentum (E4) per Stage I focus letter A. RR1+RR2 produced by RESEARCHER (not APM).
- **II Filter out:** No A&J memo. APM consults Stream A § 1 Funnel view. Filter out on E5 (thematic fit) + E11 (VC dynamics). RR3+RR4 produced by RESEARCHER.
- **III Triaging:** **First A&J pass.** APM produces memo per the per-row authoring matrix at column III. In scope: rows 48 (exec summary, 3rd pass), 49 (master ratings, 1st pass — light), Pillar II group ratings (1st pass — light), E1, E5 (2nd pass), E7, E8, E11 (2nd pass). Verdict: progress / park / kill.
- **IV ESA:** **Second A&J pass.** Memo deepens per matrix column IV. Pillar I solidifies (E1 2nd, E2+E3 1st pass). Pillar II Core philosophy group fills in (E9–E13 1st pass). Pillar II Risk transmission (E18, E19) 1st pass — light. Pillar III begins: G6 Position playbook, G7 Upstream, G8 Downstream all 1st pass — light. Verdict: progress / park / kill.
- **V DD:** **Third A&J pass.** Memo at robust depth. Risk attributes (E15–E17) **first authored at DD only** (not earlier). E6 SS earnings momentum 1st pass at DD. Pillar III deepens to 2nd pass — solid. Verdict: progress / park / kill / size.
- **VI Live:** No new A&J memo authoring; instead, the **Stage VI Live operating system** runs: weekly RR23 monitoring + monthly review meeting (Richard + APM) + 10 INVALIDATION ACHs as exit-trigger framework. Pillar III evolves to 3rd pass — robust as new monitoring data accumulates.

**Stage gates are NOT programmatic** [W4]. TCs are qualitative alignment language; APM does NOT rate the TCs. Go/no-go = combination of element/group/pillar/master ratings + integrated APM judgement. Per `STREAM-A-COMPREHENSIVE-V2.html` § 2.

**For full Funnel view** (stages × focus letters × A&J elements × research reports, at-a-glance) see `STREAM-A-COMPREHENSIVE-V2.html` § 1.
